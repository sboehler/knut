	...
	day := func(y int, m time.Month, d int, v int64, trx ...*ast.Transaction) *ast.Day {
		return &ast.Day{
			...
			Value: amounts.Amounts{
				amounts.AccountCommodityKey(td.account1, td.commodity1): decimal.NewFromInt(v),
			},
		}
	}
			...
			},
			{
				...
				want: []*ast.Period{
					...
					},
					{
						...
						Values: amounts.Amounts{
							amounts.AccountCommodityKey(td.account1, td.commodity1): decimal.NewFromInt(300),
						},
						PrevValues: amounts.Amounts{
							amounts.AccountCommodityKey(td.account1, td.commodity1): decimal.NewFromInt(100),
						},
